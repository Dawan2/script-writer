 ...
 * - 错误统一 throw SwError（SW-E010/SW-E031），由接口层顶层 catch 渲染并退出码 1（GAP-06）。
 */
...
import { CHARACTERS_DIR, EXPORTS_DIR, OUTLINE_FILE, SCENES_DIR } from '../../infra/store/layout.js';
import {
  PROJECT_FILE,
  inspectDir,
...
import { SwError } from '../errors/sw-error.js';
...
function failTargetNotEmpty(target: string, dirArg: string | undefined): never {
  const dirHint = dirArg ?? '<dir>';
  throw new SwError({
    code: 'SW-E010',
    what: '目标目录非空',
    why: `${target} 已存在且包含文件，直接初始化可能覆盖既有内容。`,
    how:
      `换一个目录，或确认覆盖后运行 \`sw init ${dirHint} --force\`` +
      `（后果：同名脚手架文件 ${PROJECT_FILE} / ${OUTLINE_FILE} / .gitignore 将被覆盖，其余文件保留）。`,
  });
}

function failTargetIsFile(target: string): never {
  throw new SwError({
    code: 'SW-E010',
    what: '目标路径已存在且不是目录',
    why: `${target} 是一个文件，无法作为项目目录。`,
    how: '换一个目录名，或移走该文件后重试 `sw init <dir>`。',
  });
}
...
async function resolveTemplate(
  ...
  if (explicitId !== undefined) {
    if (!available.includes(explicitId)) {
      throw new SwError({
        code: 'SW-E031',
        what: `模板不存在：${explicitId}`,
        why: `templates/ 下没有名为 ${explicitId} 的模板目录。`,
        how: `可用模板：${available.join('、')}。换用 \`--template <id>\`，或省略该旗标（默认跟随脚本类型）。`,
      });
    }
...
export async function runInitWorkflow(
  ...
  if (dirState === 'file') {
    failTargetIsFile(target);
  }
  if (dirState === 'non-empty' && !flags.force) {
    failTargetNotEmpty(target, dirArg);
  }
  ...
  const files: ProjectFileEntry[] = [
    { relPath: PROJECT_FILE, content: serializeProjectMeta(meta) },
    ...templateFiles,
  ];
