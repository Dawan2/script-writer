...
import { PROJECT_FILE, inspectDir } from '../../infra/store/projectFile.js';
import type { ProjectFileReadResult } from '../../infra/store/projectMetaRead.js';
...
export interface DoctorContext {
  ...
  nodeVersion: string;
  /** project.yaml 预读结果（一次 IO，多检查共享） */
  projectFile: ProjectFileReadResult;
...
const layoutCheck: DoctorCheck = {
  ...
  async run(ctx) {
    ...
    if (outlineMissing) {
      fixParts.push(
        `运行 \`sw init ${dirHint(ctx)} --force\` 从模板重建 ${OUTLINE_FILE}（同名脚手架文件将被覆盖，其余文件保留）`,
      );
    }
...
const scenesDoneCheck: DoctorCheck = {
  ...
  async run(ctx) {
    ...
    const missing = scenesDone.filter(
      (id) => !names.some((name) => name === `${id}.md` || (name.startsWith(`${id}-`) && name.endsWith('.md'))),
    );
    if (missing.length === 0) {
    ...
    return fail(
      ...
      `在 ${SCENES_DIR}/ 补回对应文件（如 \`${SCENES_DIR}/${missing[0]}-<slug>.md\`），或编辑 ${PROJECT_FILE} 从 progress.scenes_done 移除缺失编号（\`sw draft\` 修复路径随 W1-P1-T05 交付）`,
    );
  },
};
