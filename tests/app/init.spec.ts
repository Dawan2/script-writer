...
describe('app/workflow/init · runInitWorkflow', () => {
  ...
  it('目标目录非空且无 --force：抛 SW-E010，三段式含 --force 提示，现场不被破坏', async () => {
    ...
    // 迁移说明：原断言查 init 版 SwError.how 字段；注册表版渲染层出「怎么办」段，断言意图不变。
    expect(renderError(error as SwError)).toContain('--force');
    expect(await readFile(path.join(target, 'draft.txt'), 'utf8')).toBe('既有内容');
    expect((await readdir(target)).sort()).toEqual(['draft.txt']);
  });

  it('目标路径是文件：报 SW-E013 且不动现场（W3 集成裁定：E010 双现场拆码，见 integration-map §3-④）', async () => {
    const root = await makeTempRoot();
    ...
    expect(error).toBeInstanceOf(SwError);
    expect((error as SwError).code).toBe('SW-E013');
    expect(renderError(error as SwError)).toContain('不是目录');
  ...
  it('--template 指向不存在的模板：SW-E031 且附可用模板列表', async () => {
    ...
    // 迁移说明：可用模板列表经注册表 ctx.available 渲染进「怎么办」段。
    expect(renderError(error as SwError)).toContain('short-video');
  });
