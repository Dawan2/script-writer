 ...
 * 对构建产物 dist/cli/main.js 逐用例 spawn 真实进程，断言退出码 0/2：
 * 退出码 1 的进程级断言随首个真实业务命令（W1-P1-T04/T05）落地——
 * 当前 CLI 尚无可触发 SW-Exxx 的子命令，框架级映射已由 tests/cli/run.spec.ts 全覆盖。
 */
...
let failed = 0;
for (const { args, expected, note } of CASES) {
...
if (failed > 0) {
  ...
  console.log(`✔ 退出码冒烟通过：${CASES.length}/${CASES.length}（SPEC-03-EXT 0/2 档）`);
}
