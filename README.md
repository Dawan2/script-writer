...
> 当前进度：CLI 入口（`sw --help` / `sw --version`）、**`sw init` 向导**与 **`sw status`（最小版）已可用**，
> 其余子命令**规划中**（逐命令实现进度见 [docs/quickstart.md](docs/quickstart.md)）。
...
创作一个脚本项目（`sw init` / `sw status` 已可用；`sw export` **规划中**，随 W1-P1-T05 后续落地）：

```bash
sw init my-story    # 交互向导（≤ 4 问），产出可续写的项目脚手架 [可用]
sw status           # 随时找回："你在第几步、下一步敲什么命令"（末行可直接复制执行）[可用]
sw export           # 导出成稿 [规划中]
```
