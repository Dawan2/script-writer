...
export async function getWorldViewExecutionStrategy(workspace, options = {}) {
  const workspaceDir = path.resolve(workspace);
  const userInput = JSON.parse(
    await fs.readFile(path.join(workspaceDir, "1.1-user-input.json"), "utf8")
  );
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    const result = await getWorldViewExecutionStrategy(parseArgs(process.argv.slice(2)));
    ...
    const nextAction = /剧本设定尚未完成/u.test(message)
      ? "先按剧本设定解析原则补全自动适配标签，再重新调用执行策略。"
      : /执行规范/u.test(message)
        ? "先重新调用初始化世界观并阅读执行规范，再重新调用执行策略。"
        : "修复返回问题后重新调用执行策略。";
